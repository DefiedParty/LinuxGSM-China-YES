...
curl -IsfL "https://gitee.com/${githubuser}/${githubrepo}/raw/${githubbranch}/linuxgsm.sh" 1>/dev/null
if [ $? != "0" ]; then
...
if [ "${remotereponame}" == "GitHub" ]; then
	tmp_script_diff=$(diff "${tmpdir}/linuxgsm.sh" <(curl -s "https://gitee.com/${githubuser}/${githubrepo}/raw/${githubbranch}/linuxgsm.sh"))
else
...
if [ "${remotereponame}" == "GitHub" ]; then
	curl -IsfL "https://gitee.com/${githubuser}/${githubrepo}/raw/${githubbranch}/lgsm/config-default/config-lgsm/${gameservername}/_default.cfg" 1>/dev/null
else
...
if [ "${remotereponame}" == "GitHub" ]; then
	config_file_diff=$(diff "${configdirdefault}/config-lgsm/${gameservername}/_default.cfg" <(curl -s "https://gitee.com/${githubuser}/${githubrepo}/raw/${githubbranch}/lgsm/config-default/config-lgsm/${gameservername}/_default.cfg"))
else
...
if [ -n "${functionsdir}" ]; then
	if [ -d "${functionsdir}" ]; then
		...
		do
			...
			if [ "${remotereponame}" == "GitHub" ]; then
				curl -IsfL "https://gitee.com/${githubuser}/${githubrepo}/raw/${githubbranch}/${github_file_url_dir}/${functionfile}" 1>/dev/null
			else
			...
			if [ $? != 0 ]; then
				...
				# compare file
				if [ "${remotereponame}" == "GitHub" ]; then
					function_file_diff=$(diff "${functionsdir}/${functionfile}" <(curl -s "https://gitee.com/${githubuser}/${githubrepo}/raw/${githubbranch}/${github_file_url_dir}/${functionfile}"))
				else
